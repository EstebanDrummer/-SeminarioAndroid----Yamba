	...
	private static final String TAG = YambaWidget.class.getSimpleName();
	    ...
	    try {
	      if (c.moveToFirst()) { // <4>
	        ...
	        for (int appWidgetId : appWidgetIds) { // <6>
	          ...
	          RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.yamba_widget); // <7>
	  ...
	  public void onReceive(Context context, Intent intent) { // <11>
	    ...
	    if (intent.getAction().equals(UpdaterService.NEW_STATUS_INTENT)) { // <12>
	      ...
	      this.onUpdate(context, appWidgetManager, appWidgetManager.getAppWidgetIds(new ComponentName(context, YambaWidget.class))); // <14>
	    }
	  }
